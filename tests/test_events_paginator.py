...
from events_aggregator.clients.events_provider import EventsProviderClient, EventsPaginator
import pytest
...
test_dict = {
	...
	"results": [
		...
		{
			"id": "550e8400-e29b-41d4-a716-4466554400321",
			"name": "Разбор ошибок"
		}
	]
}
...
first_pages = {
	...
	"results": [
		{
			"id": "550e8420-e39b-41d4-a716-4426554400321",
			"name": 'Жарим пирожки'
		}
	]
}
...
async def test_paginator_skips_empty_page():
	fake_client = Mock(spec=EventsProviderClient)
	fake_client.events = AsyncMock(return_value=first_pages)
	fake_client.get_page = AsyncMock(side_effect=[empty_middle_page,
	                                              third_pages])
	paginator = EventsPaginator(fake_client, date="2026-01-05")
	events = []
	async for event in paginator:
	...
	assert events == first_pages["results"] + empty_middle_page["results"] + third_pages["results"]
	fake_client.events.assert_awaited_once_with("2026-01-05")
