...
async def test_register_returns_string_and_calls_provider():
	with patch("events_aggregator.clients.events_provider.httpx.AsyncClient") as fake_client:
		...
		fake_json = {
			"ticket_id": "4baef786-4578-432e-9927-7f721d1c0be2"
		}
		fake_body = {
			"first_name": "Иван",
			"last_name": "Иванов",
...
async def test_unregister_returns_string_and_calls_provider():
	with patch("events_aggregator.clients.events_provider.httpx.AsyncClient") as fake_client:
		...
		fake_response.json.return_value = fake_json
		result = await client.unregister(event_id="event-uuid",
		                                 ticket_id="1fed0122-b675-42e2-8ae7-49bfb53e8d7f")
		assert result == fake_json
		fake_response.raise_for_status.assert_called_once_with()
		mock_http_client.request.assert_awaited_once_with("DELETE",
			 "/api/events/event-uuid/unregister/", json=fake_body
		)